...
<?=bitrix_sessid_post()?>
	<div class="row">
		<div class="col-12 col-sm-6">
			<label><?=GetMessage("MFT_NAME")?><?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("NAME", $arParams["REQUIRED_FIELDS"])):?> <span class="text-danger">*</span><?endif?></label>
			 <input type="text" name="user_name" value="<?=$arResult["AUTHOR_NAME"]?>" maxlength="12"/>
		</div>
	</div>
	<div class="row">
		<div class="col-12 col-sm-6">
			<label><?=GetMessage("MFT_TEL")?><?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("TEL", $arParams["REQUIRED_FIELDS"])):?> <span class="text-danger">*</span><?endif?></label>
			  <input type="text" name="user_tel" value="<?=$arResult["AUTHOR_TEL"]?>" class="js-validate-phone" placeholder="+7 (___) ___-__-__">
		</div>
	</div>
	...
	<div class="row">
		...
		<div class="col-12">
			...
			<img src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult["capCode"]?>" width="180" height="40" alt="CAPTCHA" title="CAPTCHA" class="captcha"/>
		</div>
		<div class="col-12">
		...
		<div class="col-12">			
			...
			<input type="submit" name="submit" value="Отправить" class="agree-button btn btn-primary" onclick="ym(56550514,'reachGoal','submit'); return true;"/>
		</div>
	</div>
</form>
</div>