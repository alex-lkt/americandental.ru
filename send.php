...
<html class="wide smoothscroll wow-animation" lang="ru">
  <head>
    ...
    if (!empty($Input)) {
        $mess .= "Вопрос: " . $Input . "<br>";
    }

    if (isset($_POST['tildaspec-formname'])) {
        $mess .= "---------------------------" . "<br>";
        $mess .= "Опрос из 4 вопросов. Протезирование" . "<br>";
        foreach ($_POST as $key => $value) {
            if (
                    $key != "formservices" AND
                    $key != "tildaspec-formname" AND
                    $key != "name" AND
                    $key != "phone" AND
                    $key != "form-spec-comments"
            ) {
                $mess .= $key . ": " . trim(strip_tags($value)) . "<br>";
            }
        }
    }
